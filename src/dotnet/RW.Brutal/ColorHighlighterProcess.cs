...
public class ColorHighlighterProcess : CSharpIncrementalDaemonStageProcessBase
{
    ...
    private static IColorReference? GetColorReference(ITreeNode element, IEnumerable<IColorReferenceProvider> providers)
    {
        ...
        {
            var ref1 = ReferenceFromInvocation(qualifier, referenceExpression);
            var reference = ref1 ?? ReferenceFromProperty(qualifier, referenceExpression);
            if (reference != null)
                return reference;
        }
    ...
    /// </summary>
    private static IColorReference? ReferenceFromProperty(
        IReferenceExpression qualifier,
        IReferenceExpression colorQualifiedMemberExpression)
    {
        ...
        return new ColorReference(
            colorElement,
            colorQualifiedMemberExpression,
            colorQualifiedMemberExpression,
            colorQualifiedMemberExpression.NameIdentifier.GetDocumentRange());
    }
